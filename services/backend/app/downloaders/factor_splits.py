...
def business_conditions(future, start_date, end_date):
    ...
    dfm[column] = np.sign(
        ...
        - dfm.CLOSE.shift(shift_period).rolling(window=window_long).mean()).fillna(method='ffill')
    return dfm[[column]]
...
def factor_splits(future, start_date, end_date):
    ...
    for index, row in tqdm(dfm.iterrows(), total=len(dfm)):
        if pd.isna(row.BusinessConditions):
            continue
        if (previous_row is not None and row.BusinessConditions != previous_row.BusinessConditions) \
